...
# if user didn't specify where logging module is located, we use a default library folder
if (!$lib) 
{ 
	$lib = $(Join-Path $(Split-Path $(Split-Path $MyInvocation.MyCommand.Path)) "lib")  
}

# Add the lib folder to the PSModulePath
